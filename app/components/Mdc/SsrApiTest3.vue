...
const fetchUrl = computed(() => {
  ...
  } else if (path.includes('ssrapitest'))
...
const { data: rows } = await useAsyncData(
  ...
  async () => {
    ...
    console.log('--- FETCH ATTEMPT mdc/SsrApiTest3.vue ---')
...
const { data: rowItems /* , status, error */ } = await useFetch<RowItems[]>(
  fetchUrl.value, {
    ...
    transform: (
      ...
      console.log('log from ssr api test 2')
      console.log('fetchUrl.value = ', fetchUrl.value)
      return data
        ?.map(sermon => ({
          ...sermon
        })) || []
